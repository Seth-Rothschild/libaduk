...
test('clock stops when opponent leaves the room', async ({ browser }) => {
  test.setTimeout(15000);
  ...
  // White navigates away — disconnects
  await white.goto('/');

  // Black sees white go offline and clock stops
  await expect(black.locator('[data-testid="presence-white"].offline')).toBeVisible();
  await expect(black.locator('.rclock-top')).not.toHaveClass(/running/);

  // Wait past the original timeout — game should NOT end
  await black.waitForTimeout(3500);
  await expect(black.locator('.message')).toContainText('Waiting for opponent');

  // White reconnects
  await white.goto(`/play/${gameId}`);
  await expect(black.locator('[data-testid="presence-white"].online')).toBeVisible();
